...
/// printing the content's same-line trailing comments after the semicolon like Prettier:
/// `foo = 1 /* c */;` -> `foo = 1; /* c */`
///
/// Own-line comments before the semicolon are left for the next node's
/// leading-comments pass, also like Prettier.
    ...
    T: Format<'a, JsFormatContext<'a>>,
{
    fn fmt(&self, f: &mut JsFormatter<'_, 'a>) {
        ...
        if !f.source_text().bytes_contain(self.content_end, self.node_end, b';')
