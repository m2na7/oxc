...
#[test]
fn drop_dead_args_cascading_unused_param_chain() {
    // `inner`'s `_u` is unused, so a pass drops `u` from `inner(a, u)`; that
    // prunes the only read of `foo`'s `u`, upgrading foo's summary MID-RUN.
    // The upgrade must force another pass (`upsert_function_summary`) for
    // `foo(1, 2)` — visited before the hoisted declaration — to consume it.
    // Without that signal the run converges one cascade level short and only
    // a whole re-run drops the arg, tripping the idempotency assert below
    // (monitor-oxc caught this on vue-router / rollup-pluginutils).
    test(
        "if (0) dead(); foo(1, 2); function foo(a, u) { inner(a, u) } function inner(a, _u) { bar(a) }",
        "foo(1); function foo(a, u) { inner(a) } function inner(a, _u) { bar(a) }",
    );
}
