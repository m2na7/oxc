...
use crate::{
    TraverseCtx,
    keep_var::KeepVar,
    state::{FunctionSummary, PureReturn},
};
...
impl<'a> PeepholeOptimizations {
    ...
    fn try_save_pure_function(
        ...
        // Redeclarations are span-only in oxc_semantic and create no references,
        // so they're invisible to the read-only-refs check: the runtime-winning
        // declaration may be an impure one that never reaches this function.
        if !ctx.scoping().symbol_redeclarations(symbol_id).is_empty()
            || !Self::params_are_plain_identifiers(params)
            // A writable binding may be reassigned to a different (impure /
            // arg-using) function that never reaches this recorder.
            || !ctx.scoping().get_resolved_references(symbol_id).all(|r| r.flags().is_read_only())
        {
            ctx.remove_function_summary(symbol_id);
            return;
        }
        ...
        {
            Some(if body.is_empty() { PureReturn::Undefined } else { PureReturn::Opaque })
        } else {
            None
        };

        // ── `dead_arg_prefix` fact — async / generator / effectful bodies OK ──
        let dead_arg_prefix = Self::compute_dead_arg_prefix(params, scope_id, ctx);

        if pure_return.is_some() || dead_arg_prefix.is_some() {
            ctx.upsert_function_summary(
                symbol_id,
                FunctionSummary { pure_return, dead_arg_prefix },
            );
        } else {
            ctx.remove_function_summary(symbol_id);
        }
    ...
    /// The item-level half of the spec's `IsSimpleParameterList` (see
    /// `oxc_ecmascript::is_simple_parameter_list`): every parameter is a plain
    /// identifier with no default. `function foo({}) {} foo(null)` is a runtime
    /// type error, and — because in oxc a parameter default lives in
    /// `FormalParameter::initializer`, NOT an `AssignmentPattern` —
    /// `is_binding_identifier()` alone would accept `(u = g())`, whose default
    /// runs on `undefined`. Rest params are gated per-fact, not here:
    /// `pure_return` never restricted them; `compute_dead_arg_prefix` bails.
    fn params_are_plain_identifiers(params: &FormalParameters<'a>) -> bool {
        params
            .items
            .iter()
    ...
    fn compute_dead_arg_prefix(
        ...
        // own object, unrelated to this call's args. (An arrow's own scope
        // carries `ScopeFlags::Arrow`, so arrow-ness is derived rather than
        // threaded through as a parameter; a missing `scope_id` conservatively
        // falls into the non-arrow path and fails its strict-mode check.)
        let is_arrow =
            scope_id.is_some_and(|scope_id| ctx.scoping().scope_flags(scope_id).is_arrow());
        if !is_arrow {
    ...
    pub fn remove_dead_code_call_expression(expr: &mut Expression<'a>, ctx: &mut TraverseCtx<'a>) {
        ...
        let Some((is_empty_body_pure, dead_arg_prefix)) =
            ctx.state.pure_functions.get(&symbol_id).map(|summary| {
                (
                    matches!(summary.pure_return, Some(PureReturn::Undefined)),
        ...
        // O(1) out for the common case before the O(args) spread scan below:
        // nothing beyond the keep-count, nothing to drop.
        if e.arguments.len() <= n {
            return;
        }
