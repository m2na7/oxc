...
impl<'a> Traverse<'a> for Normalize {
    ...
    // The three hooks below seed `MinifierState::member_write_hazard_symbols`
    // (see its docs). Normalize always runs before the fixed-point loop, so
    // the loop starts with a program-wide, execution-order-independent view of
    // hazardous member writes. Skipped in DCE mode — the default-path drop of
    // write-only property assignments that consumes the set is full-minify
    // only.
...
impl<'a> Normalize {
    ...
    /// `MinifierState::member_write_hazard_symbols`. `object` is the member
    /// expression's object; walking it to the base identifier determines the
    /// chain depth. The hazard applies iff the op reads the property
    ...
    fn record_member_write_hazard(
        ...
        ctx.state.member_write_hazard_symbols.insert(symbol_id);
    }
