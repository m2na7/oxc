...
//!
//! [`FilesParser::get_processed_files`] then assigns [`FileId`]s in tsgo's `collectFiles` order —
//! a post-order walk of the task graph from the roots, so a file's dependencies come before it —
//! which also keeps the output deterministic despite rayon's nondeterministic arrival order.
