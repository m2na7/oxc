...
//!
//! oxc's parser already collects top-level static imports/re-exports and every dynamic `import()`
//! into the module record, so the AST walks here only add what the module record lacks:
//! `import x = require("...")`, statements nested inside ambient module bodies, `import("...")`
//! type queries, CommonJS `require("...")` calls in JavaScript files, and the ambient module
//! declarations themselves.

use oxc_ast::ast::{
    Argument, CallExpression, Declaration, Expression, Program as AstProgram, Statement,
...
pub(super) fn collect_external_module_references(
    ...
    // type-space imports): dynamic `import()`s come from the module record; `import("...")`
    // type queries and — in JavaScript files only — `require("...")` calls need an AST walk.
    let mut dynamics: Vec<(u32, CompactStr)> = module_record
        .dynamic_imports
        .iter()
        .filter_map(|import| {
            let raw = &program.source_text
                [import.module_request.start as usize..import.module_request.end as usize];
            dynamic_import_specifier(raw)
                .filter(|specifier| !specifier.is_empty())
                .map(|specifier| (import.module_request.start, CompactStr::from(specifier)))
        })
        .collect();
...
/// Collects `import("...")` type queries (tsgo's "type space imports"), which live in type
/// positions anywhere in the AST, and — in JavaScript files — `require("...")` calls.
...
impl<'a> Visit<'a> for CallCollector<'_> {
    ...
    fn visit_call_expression(&mut self, it: &CallExpression<'a>) {
        ...
        if self.collect_require_calls
            ...
            && it.arguments.len() == 1
        {
            match &it.arguments[0] {
                Argument::StringLiteral(argument) => {
                    if !argument.value.is_empty() {
                        self.dynamics
                            .push((argument.span.start, CompactStr::from(argument.value.as_str())));
                    }
                }
                Argument::TemplateLiteral(argument) if argument.is_no_substitution_template() => {
                    let value = argument.quasis[0].value.cooked.as_ref().map_or_else(
                        || argument.quasis[0].value.raw.as_str(),
                        |cooked| cooked.as_str(),
                    );
                    if !value.is_empty() {
                        self.dynamics.push((argument.span.start, CompactStr::from(value)));
                    }
                }
                _ => {}
            }
...
/// The string content of a dynamic `import()` argument, when it is string-literal-like
/// (tsgo passes `requireStringLiteralLikeArgument`: a plain string literal or a
/// no-substitution template). Escape sequences are kept raw; specifiers containing them
/// simply fail resolution, matching tsgo's unresolvable-module behavior closely enough.
fn dynamic_import_specifier(raw: &str) -> Option<&str> {
    ...
    match first {
        b'\'' | b'"' if last == first => Some(&raw[1..raw.len() - 1]),
        b'`' if last == b'`' => {
